...
class ConsoleInterface:
    ...
    def show_setting(self) -> None:
        ...
        print("\n  DISCOVERY:")
        print(
            f"    - Body discovered by {discovered_by if discovered_by else 'unknown'} at {discovery_time if discovery_time else 'unknown time'}"
        )
        if door_locked:
        ...
        if len(public_events) > 3:  # More than just basic facts
            print("\n  ADDITIONAL NOTES:")
            print("    - Multiple witnesses present at estate")
            print("    - All occupants have been secured on premises")
            print("    - Await your arrival to conduct interviews")
        ...
        if npc_name:
            ...
            for entry in schedule:
                ...
                if entry.notes and not entry.is_public:
                    ...
                    self.print_colored(
                        f"    [Private Note: {entry.notes}]", Fore.MAGENTA
                    )
        else:
            ...
            for day in sorted(days):
                ...
                for period in world.time_periods:
                    ...
                    has_entries = any(
                        any(
                            e.time_block.day == day and e.time_block.period == period
                            for e in schedules
                        )
                        for schedules in all_schedules.values()
                    )
                    ...
                    for char in sorted(characters):
                        ...
                        entry = next(
                            (
                                e
                                for e in schedules
                                if e.time_block.day == day
                                and e.time_block.period == period
                            ),
                            None,
                        )
                        ...
                        if entry:
                            companions = (
                                f" (with {', '.join(entry.with_characters)})"
                                if entry.with_characters
                                else ""
                            )
                            print(
                                f"    {char:20s} -> {entry.location:15s} - {entry.activity}{companions}"
                            )
                        else:
        ...
        if cmd in ["/quit", "/exit"]:
            ...
            if not arg:
                ...
                if npc:
                    ...
                    self.print_system(
                        f"NPC '{arg}' not found. Use /npcs to see available NPCs."
                    )
            return True
    ...
    def run(self) -> None:
        ...
        while self.running:
            try:
                ...
                if self.current_npc:
                    prompt = (
                        f"{Fore.YELLOW}[Talking to {self.current_npc}]{Style.RESET_ALL} > "
                        if COLORS_ENABLED
                        else f"[{self.current_npc}] > "
                    )
                else:
                    prompt = (
                        f"{Fore.CYAN}[No NPC selected]{Style.RESET_ALL} > "
                        if COLORS_ENABLED
                        else "> "
                    )
                ...
                if not self.current_npc:
                    self.print_system(
                        "Please select an NPC first using /talk <npc_name>"
                    )
                    continue
                ...
                response, metadata = self.engine.converse(
                    self.current_npc, user_input, self.player_name
                )
...
def main():
    ...
    try:
        ...
        print(
            "No NPCs loaded. You can add them programmatically or create example_scenario.py"
        )
