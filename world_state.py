...
class Event(BaseModel):
    ...
    sequence_order: int = (
        0  # Order within the same time period (0 = first, 1 = second, etc.)
    )
...
class WorldState:
    ...
    def __init__(self):
        ...
        self.npc_schedules: Dict[
            str, List[NPCScheduleEntry]
        ] = {}  # character -> list of schedule entries
        self.time_periods = [
            "early_morning",
            "morning",
            "noon",
    ...
    def add_fact(
        ...
        responsible_npc: Optional[str] = None,
    ) -> None:
    ...
    def query_facts(
        self, category: Optional[str] = None, is_public: Optional[bool] = None
    ) -> List[Fact]:
    ...
    def get_relationship_between(self, char_a: str, char_b: str) -> List[Relationship]:
        ...
        return [
            r
            for r in self.relationships
            if (r.character_a == char_a and r.character_b == char_b)
            or (r.character_a == char_b and r.character_b == char_a)
        ]
    ...
    def add_schedule_entry(
        ...
        if period not in self.time_periods:
            raise ValueError(
                f"Invalid period '{period}'. Must be one of: {self.time_periods}"
            )
    ...
    def get_character_schedule(
        self, character: str, day: Optional[int] = None
    ) -> List[NPCScheduleEntry]:
        ...
        return sorted(
            ...
            key=lambda e: (
                e.time_block.day,
                self.time_periods.index(e.time_block.period),
            ),
        )
    ...
    def get_character_location_at_time(
        self, character: str, day: int, period: str
    ) -> Optional[str]:
    ...
    def get_characters_at_location_time(
        self, location: str, day: int, period: str
    ) -> List[str]:
    ...
    def verify_character_claim_time_location(
        self, character: str, claimed_location: str, day: int, period: str
    ...
    def get_world_summary(self) -> Dict[str, Any]:
        ...
        total_schedule_entries = sum(
            len(entries) for entries in self.npc_schedules.values()
        )
    ...
    def export_character_knowledge(
        self, character: str, current_scene: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        if current_scene:
            ...
            relevant_chars = [
                c.lower() for c in self.characters if c.lower() in scene_lower
            ]
            relevant_locs = [
                loc.lower() for loc in self.locations if loc.lower() in scene_lower
            ]

            def is_relevant(text: str) -> bool:
                t = text.lower()
                return any(c in t for c in relevant_chars) or any(
                    l in t for l in relevant_locs
                )
            ...
            if relevant_chars or relevant_locs:
                known_facts = [
                    ...
                    if is_relevant(str(f.value)) or is_relevant(f.key)
                ]
                known_events = [
                    e
                    for e in known_events
                    if is_relevant(e.description)
                    or is_relevant(e.location)
                    or any(c.lower() in scene_lower for c in e.participants)
                ]
                relationships = [
                    r
                    for r in relationships
                    if r.character_a.lower() in scene_lower
                    or r.character_b.lower() in scene_lower
                ]
