...
def test_character_knows_private_fact_if_witnessed(populated_world):
    populated_world.add_fact(
        "secret_meeting", True, is_public=False, witnesses=["Alice"]
    )
...
def test_add_and_get_relationships(populated_world):
    populated_world.add_relationship(
        "Alice", "Bob", "friends", "They have been friends since kids", strength=8
    )
    populated_world.add_relationship(
        "Alice", "Charlie", "enemies", "Rivals", strength=2
    )
...
def test_set_and_query_npc_schedule(populated_world):
    populated_world.add_schedule_entry(
        "Alice", 1, "morning", "Park", "reading", is_public=True
    )
    populated_world.add_schedule_entry(
        "Alice", 1, "afternoon", "Library", "studying", is_public=False
    )
...
def test_verify_character_claim_time_location(populated_world):
    ...
    is_valid, actual_loc = populated_world.verify_character_claim_time_location(
        "Bob", "Library", 1, "morning"
    )
...
def test_character_knows_schedule_of_others_if_public(populated_world):
    ...
    populated_world.add_schedule_entry(
        "Bob", 1, "morning", "Library", "reading", is_public=True
    )
    populated_world.add_fact(
        "bob_schedule_d1_morning",
        True,
        is_public=True,
        schedule_day=1,
    ...
    populated_world.add_schedule_entry(
        "Charlie", 1, "afternoon", "Park", "walking", is_public=False
    )
    populated_world.add_fact(
        "charlie_schedule_d1_afternoon",
        True,
        is_public=False,
        schedule_day=1,
        schedule_period="afternoon",
    )

    # Alice should know Bob's public schedule
    assert (
        populated_world.character_knows_fact("Alice", "bob_schedule_d1_morning") is True
    )
    # Alice should NOT know Charlie's private schedule
    assert (
        populated_world.character_knows_fact("Alice", "charlie_schedule_d1_afternoon")
        is False
    )
